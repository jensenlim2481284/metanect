...
// Nav item animation 
$(document).on('mouseover','.nav-btn',function(){
...
setTimeout(() => {
  const img = new AnimatedImage('/img/logo/logo.png');
...
setTimeout(() => {
  const text = new AnimatedText('Metanect', fontAsset);
...
setTimeout(() => {
  const text = new AnimatedText('The future of E-commerce - stay collaborative not competitive', fontAsset, 0.125);
  text.position.x -= text.basePosition * 0.5;
  text.position.y -= 0.8;
  webgl.add(text);
  text.show();
